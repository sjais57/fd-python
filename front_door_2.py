...
class UnifiedFrontDoorService:
    ...
    async def initialize(self):
        ...
        if OBSERVABILITY_AVAILABLE:
            ...
            # Global metrics service uses default registry
            self.global_metrics_service = MetricsService()  # Uses default Prometheus registry
    ...
    async def configure_observability_from_manifest(self, project_id: str, manifest: Dict[str, Any]):
        ...
        for module in modules:
            ...
            module_config = module.get("config", {}).copy()  # Create a copy to modify
            ...
            if module_type == "health_check":
                ...
                # Ensure unique namespace for project metrics
                if "namespace" not in module_config:
                    module_config["namespace"] = f"{project_id}_metrics"
                else:
                    # Prefix with project_id to ensure uniqueness
                    module_config["namespace"] = f"{project_id}_{module_config['namespace']}"
                
                logger.info(f"Creating metrics service for project {project_id} with namespace: {module_config['namespace']}")
                
                # Create metrics service from manifest config WITH CUSTOM REGISTRY
                metrics_service = MetricsService.from_manifest_config(
                    module_config, 
                    use_custom_registry=True  # Isolated registry for project
                )
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/health") or path.startswith("/metrics") or "/health" in path or "/metrics" in path:
        return await call_next(request)
    ...
    try:
        ...
        # Use project-specific metrics service if available
        if project_id and project_id in app.state.front_door.metrics_services:
            ...
            logger.debug(f"Using project-specific metrics service for {project_id}")
        else:
            # Use global metrics service for unconfigured projects
            metrics_service = app.state.front_door.global_metrics_service
            logger.debug(f"Using global metrics service for {project_id}")
    except Exception as e:
        logger.warning(f"Failed to determine metrics service: {e}")
        metrics_service = app.state.front_door.global_metrics_service
...
async def project_metrics(project_id: str):
    ...
    # Don't allow accessing global metrics via project path
    if project_id == "_global":
        raise HTTPException(status_code=404, detail="Use /metrics for global metrics")
